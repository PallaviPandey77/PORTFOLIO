...
# Simulated JSON data as an example
response_data = '''
{
   "cod": "200",
   "message": 0.0151,
...
def print_temperature(date):
    forecast = get_weather_by_date(date)
    if forecast:
        temp_celsius = forecast["main"]["temp"] - 273.15
        print(f"Temperature for {date}: {temp_celsius:.2f}°C")
    else:
...
def print_wind_speed(date):
    ...
    if forecast:
        ...
        print(f"Wind Speed for {date}: {wind_speed} m/s")
    else:
        print("No data available for the provided date.")
...
def print_pressure(date):
    ...
    if forecast:
        ...
        print(f"Pressure for {date}: {pressure} hPa")
    else:
        print("No data available for the provided date.")
